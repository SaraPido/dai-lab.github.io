...
def get_dates_weekly():
    dates = []
    start_date = datetime.datetime.strptime(CONT_START_DATE, "%Y-%m-%d")
...
for repo in public_repos:
    ...
    if now_str in dates_weekly:
        DATA_METRIC_WEEKLY = DATA_JSON
        #DATA_METRIC_WEEKLY[repo_full_name]["stargazers"] = DATA_METRIC_WEEKLY[repo_full_name]["stargazers"]["totalCount"]
        #DATA_METRIC_WEEKLY[repo_full_name]["watchers"] = DATA_METRIC_WEEKLY[repo_full_name]["watchers"]["totalCount"]
    # MONTHLY
    if now_str in dates_monthly:
        DATA_METRIC_MONTHLY = DATA_JSON
        #DATA_METRIC_MONTHLY[repo_full_name]["stargazers"] = DATA_METRIC_MONTHLY[repo_full_name]["stargazers"]["totalCount"]
        #DATA_METRIC_MONTHLY[repo_full_name]["watchers"] = DATA_METRIC_MONTHLY[repo_full_name]["watchers"]["totalCount"]    
...
# Save to _data Weekly data
if DATA_METRIC_WEEKLY:
    file_path = PATH_TO_DATA + "/" + "weekly.json"
    with open(file_path, "w+") as f:
        json.dump(DATA_METRIC_WEEKLY, f)
    print("LOG: Saved to", file_path)
# Save to _data Monthly data
if DATA_METRIC_MONTHLY:
    file_path = PATH_TO_DATA + "/" + "monthly.json"
    with open(file_path, "w+") as f:
        json.dump(DATA_METRIC_MONTHLY, f)
    print("LOG: Saved to", file_path)
